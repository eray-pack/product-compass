...
function LifeTreePage({
  ...
  async function generateTreeArt() {
    ...
    try {
      const res = await fetch("/api/generate-tree", {
        ...
        body: JSON.stringify({ stage: stage.stage, stageName: stage.name }),
      });
      const data = await res.json() as { url?: string; error?: string };
      if (!res.ok || !data.url) throw new Error(data.error ?? "Generation failed");
      setArtUrl(data.url);
    } catch (e) {
      setArtError(e instanceof Error ? e.message : "Something went wrong");
    } finally {
      setArtLoading(false);
    }
  }
  ...
  return (
    <PageShell>
      ...
      <section className="px-6 mt-3">
        <motion.button
          whileTap={{ scale: 0.96 }}
          onClick={generateTreeArt}
          disabled={artLoading}
          style={{
            width: "100%", padding: "14px 20px", borderRadius: 16,
            display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
            background: artLoading
              ? "rgba(255,255,255,0.04)"
              : "radial-gradient(ellipse at 20% 50%, rgba(201,168,76,0.18) 0%, transparent 70%), rgba(255,255,255,0.04)",
            border: artLoading ? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(201,168,76,0.35)",
            cursor: artLoading ? "default" : "pointer",
            transition: "all 0.2s",
          }}
        >
          {artLoading ? (
            ...
            <>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: "#f5ede0" }}>Generate my tree art</span>
              <span style={{ fontSize: 10, color: "rgba(201,168,76,0.7)", marginLeft: 2 }}>AI · Flux</span>
            </>
          )}
        </motion.button>
        {artError && (
          <p style={{ fontSize: 11, color: "#ff7777", textAlign: "center", marginTop: 8 }}>{artError}</p>
        )}
      </section>
      ...
      {artUrl && (
        ...
        >
          <motion.div
            ...
            style={{ width: "100%", maxWidth: 360, borderRadius: 24, overflow: "hidden", boxShadow: "0 32px 80px rgba(0,0,0,0.7)" }}
          >
            <img src={artUrl} alt="Your tree art" style={{ width: "100%", display: "block" }} />
            <div style={{
          ...
          <p style={{ fontSize: 11, color: "rgba(255,255,255,0.25)", marginTop: 16 }}>Tap outside to close</p>
        </motion.div>
      )}
